...
let SearchService = class SearchService {
    ...
    async searchProducts(params) {
        ...
        let collectionRef = this.db.collection('products')
            .where('status', '==', 'ACTIVE');
        ...
        if (query && query.length >= 2) {
            ...
            if (minPrice !== undefined)
                collectionRef = collectionRef.where('basePrice', '>=', Number(minPrice));
            if (maxPrice !== undefined)
                collectionRef = collectionRef.where('basePrice', '<=', Number(maxPrice));
        ...
        return {
            results: snapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data()))),
