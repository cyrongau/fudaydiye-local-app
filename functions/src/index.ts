...
import { PaymentFactory } from "./services/payment/PaymentService";

admin.initializeApp();
const db = admin.firestore();

// Minimal Interfaces to avoid shared dependency complexity
interface CartItem {
    productId: string;
    variationId?: string;
    qty: number;
...
export const createOrder = functions.https.onCall(async (data: CreateOrderPayload, context) => {
        ...
        const result = await db.runTransaction(async (transaction) => {
            ...
            for (const item of cartItems) {
                ...
                if (item.variationId && productData.variations) {
                    ...
                    if (!variant) throw new functions.https.HttpsError("not-found", `Variant ${item.variationId} not found.`);
            ...
            const orderData = {
                ...
                items: orderItems, // Trusted items with server prices
                total: finalTotal,
                ...
                paymentMask: paymentMethod === 'CARD' ? `**** **** ${data.paymentDetails?.last4}` : recipientPhone,
                currency: 'USD',
            ...
            if (recipientId && savePayment && !recipientId.startsWith("guest_") && paymentMethod === 'CARD') {
                ...
                transaction.update(userRef, {
                    savedPaymentSources: admin.firestore.FieldValue.arrayUnion({
                        ...
                        mask: `**** ${data.paymentDetails?.last4}`,
                        lastUsedAt: new Date()
                    })
                });
            }
    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            ...
            await smsService.send({
                to: "DO_NOT_SEND_REAL", // Mocked anyway by default
...
export const verifyOtp = functions.https.onCall(async (data, context) => {
    ...
    else query = usersRef.where('phoneNumber', '==', identifier); // Ensure field name matches registration
...
export const initiatePayment = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const request = {
            orderId,
            amount: order.total, // Trust the server-side stored total
            currency: 'USD',
        ...
        if (result.success) {
            await orderRef.update({
                paymentStatus: result.status, // PENDING or COMPLETED
                paymentGatewayRef: result.transactionId || null,
                paymentMethod: paymentMethod, // Confirm the method used
