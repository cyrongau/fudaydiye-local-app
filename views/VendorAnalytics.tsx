...
const VendorAnalytics: React.FC = () => {
   ...
   const [realStats, setRealStats] = useState({
      revenue: 0,
      orders: 0,
      liveReach: 0, // Placeholder as we don't have this metric yet
      returns: 0
   });
   ...
   useEffect(() => {
      ...
      const unsub = onSnapshot(qOrders, (snap) => {
         ...
         snap.docs.forEach(doc => {
            ...
            // Revenue: Only Delivered or Completed
            if (['DELIVERED', 'COMPLETED', 'ACCEPTED'].includes(status)) {
               rev += total;
            }
            count++;
            if (status === 'RETURNED' || status === 'REFUNDED') ret++;
         });

         setRealStats({
            revenue: rev,
            orders: count,
            liveReach: count * 12, // Estimated impression multiplier
            returns: count > 0 ? (ret / count) * 100 : 0
         });
      }, (error) => {
   ...
   const stats = [
      { label: "Net Revenue", value: `$${realStats.revenue.toLocaleString()}`, trend: "+24%", icon: "payments", color: "text-primary" },
      { label: "Active Orders", value: realStats.orders.toString(), trend: "+12%", icon: "package_2", color: "text-blue-500" },
      { label: "Live Reach", value: realStats.liveReach.toLocaleString(), trend: "+42%", icon: "visibility", color: "text-amber-500" },
      { label: "Return Rate", value: `${realStats.returns.toFixed(1)}%`, trend: "-5%", icon: "assignment_return", color: "text-purple-500" },
   ];
