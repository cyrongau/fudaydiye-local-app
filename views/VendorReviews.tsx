...
const VendorReviews: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const average = total > 0 ? data.reduce((acc, curr) => acc + curr.rating, 0) / total : 0;
      const respondedCount = data.filter(r => r.responded).length;
      ...
      setStats({
        ...
        fulfillmentScore: total > 0 ? 95 : 0 // Placeholder/Estimate until we have order linkage
      });
      setLoading(false);
    });
        ...
        <section className="bg-primary/5 rounded-[32px] p-6 border border-primary/20 flex items-center gap-5 relative overflow-hidden group">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-gray-500 dark:text-gray-400 leading-relaxed uppercase tracking-widest">
              {stats.total > 10 ?
                "Analysis pending... AI is processing your latest feedback trends." :
                "Collect at least 10 verified reviews to unlock AI-powered sentiment analysis."}
            </p>
          </div>
        </section>
